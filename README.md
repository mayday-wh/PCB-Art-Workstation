...
> 注意：如果使用 **立创EDA** 导出模式，工具会自动完成底层左右翻转和黑白反相，并导出到 `立创EDA` 文件夹。
...
- 重做顶部自绘标签页，切换更稳定
- 优化按钮、复选项和层设置为圆角色块风格
- 收紧色卡录入区和色彩映射区布局，给预览区留出更多空间
- 色彩映射行改为连续紧凑布局，并统一使用 32x32 色块
- 默认使用蓝色阻焊 + 有背光模式
- 新增 **立创EDA** 导出模式：自动处理底层左右翻转、角标、黑白反相，并导出到 `立创EDA` 文件夹
- **立创EDA** 模式默认开启，普通导出逻辑仍保持原有空层/全覆盖层过滤规则
