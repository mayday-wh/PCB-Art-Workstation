...
从 [Releases](../../releases) 页面下载最新版本的压缩包，解压后双击 `.exe` 即可运行。压缩包中的 `colors.json` 是色卡数据库，请和 `.exe` 放在同一目录下。使用方法见下文的功能模块介绍。
...
首次运行软件时，会在 `.exe` 所在目录生成 `colors.json` 文件，用于存储色卡信息。每条色卡会绑定阻焊颜色（绿、蓝、红、紫、黄、白、黑）和展示模式（有/无背光）。

[Releases](../../releases) 页面提供的软件压缩包已包含录入了部分色卡信息的 `colors.json` 文件。
...
打包成功后，产物会生成在 `dist` 目录中：

```text
PCB_Art_Assistant_v4.0.exe
colors.json
```
